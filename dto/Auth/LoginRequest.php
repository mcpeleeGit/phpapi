...
class LoginRequest {
    ...
    private function validate() {
        ...
        if (!$this->request->has('email') || !$this->request->has('password')) {
            throw new ValidationException('필수 필드가 누락되었습니다.', 400, 'LOGIN_REQUEST_MISSING_FIELDS');
        }
        
        // 이메일 형식 검증
        if (!filter_var($this->request->get('email'), FILTER_VALIDATE_EMAIL)) {
            throw new ValidationException('올바른 이메일 형식이 아닙니다.', 400, 'LOGIN_REQUEST_INVALID_EMAIL');
        }
        
        // 비밀번호 길이 검증
        if (strlen($this->request->get('password')) < 8) {
            throw new ValidationException('비밀번호는 8자 이상이어야 합니다.', 400, 'LOGIN_REQUEST_INVALID_PASSWORD');
        }
